...
function [px, py, threshold]= threshold_pxpy_v1(latent,psf_size,opts,threshold)
...
[px,py] = gradient(denoised);
%  latent_x0=net_compute(denoised,opts.net_x,opts.usegpu);
%  px=latent_x0;
% %  px=L0Smoothing(latent_x0,7e-5);
%  latent_y0=net_compute(denoised,opts.net_y,opts.usegpu);
% %  py=L0Smoothing(latent_y0,7e-5);
% py=latent_y0;
pm = px.^2 + py.^2;
